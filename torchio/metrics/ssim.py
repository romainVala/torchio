...
class SSIM3D(MapMetric):
    ...
    def __init__(self, metric_name="SSIM", k1=.01, k2=.01, k3=.01, L=None, alpha=1, beta=1, gamma=1, kernel="uniform", sigma=3.0,
                 kernel_size=3, truncate=3, **kwargs):
    ...
    def apply_metric(self, sample1: Subject, sample2: Subject):
        ...
        for sample_key in common_keys:
            ...
            computed_metrics = functional_ssim(data1, data2, k1=self.k1, k2=self.k2, k3=self.k3,
                                                    ...
                                                    kernel=self.kernel, sigma=self.sigma, kernel_size=self.kernel_size,
                                                    truncate=self.truncate, return_map=True)
...
def functional_ssim(x, y, k1=.01, k2=.01, k3=.01, L=None, alpha=1, beta=1, gamma=1, kernel="uniform", sigma=3.0,
                    kernel_size=3, truncate=3, return_map=False, clamp_ssim = False):
    """
    ...
    std_xy = spatial_filter_nd(torch.mul(x_sub, y_sub),  kernel=kernel_params)
    ...
    l2_xy = spatial_filter_nd((x_sub - y_sub).pow(2), kernel=kernel_params).sqrt()
